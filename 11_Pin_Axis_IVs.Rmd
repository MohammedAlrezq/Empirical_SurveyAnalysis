...
psych::print.psych(model_11_IV_F5_9items, sort=TRUE)
...
# Number of factors is 5 factors. Uisng Oblique "cluster" and another package 
```{r}
# with factor = 5
set.seed(2323300)
...
library(EFAtools)
EFA_VDs <- EFAtools::EFA(model_11_IVs, n_factors = 6, method = "PAF", rotation = "oblimin", normalize = FALSE )

Nfactro <- N_FACTORS(model_11_IVs, criteria = c("CD", "EKC", "HULL", "KGC", "PARALLEL", "SCREE", "SMT"), method = "PAF", N_samples = 67)
...
Check Cronbach’s Alpha for 5 latent factors 
...
latent_factr_3_5 <- alpha(model_11_IVs[, c(11, 6:8)]) # 0.84, if item 8 dropped, then cronach alph improve to 0.86

latent_factr_4_5 <- alpha(model_11_IVs[, c(5, 9, 1)]) # 0.89

latent_factr_5_5 <- alpha(model_11_IVs[, c(2, 17)]) # 0.82
