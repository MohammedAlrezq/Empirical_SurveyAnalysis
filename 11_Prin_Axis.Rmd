...
```
```{r}
# get the lower triangle of the correlation matrix, as the matrix is symmetric
lower_tri <- model_11_Corr |> 
  lower.tri(diag = FALSE)

lower_tri <- model_11_Corr |> 
  lower.tri(diagonal = FALSE)

# find the variables that have a correlation coefficient greater than 0.5
high_cor_vars <- which(abs(lower_tri) > 0.5, arr.ind = TRUE) %>%
  rownames()
```
```{r}

# convert your correlation data frame to a matrix
model_11_matrix <- as.matrix(model_11_Corr)

# set the diagonal of the correlation matrix to NA
diag(model_11_matrix) <- NA

# get the lower triangle of the matrix
lower_tri <- model_11_matrix[lower.tri(model_11_matrix)]

# adjust the correlation threshold here
cor_threshold <- 0.7  # or whatever value you choose

# find the variables that have a correlation coefficient greater than the threshold
high_cor_indices <- which(abs(lower_tri) > cor_threshold, arr.ind = TRUE) 

# get the variable names
high_cor_vars <- colnames(model_11_matrix)[unique(c(high_cor_indices[,1], high_cor_indices[,2]))]
```
